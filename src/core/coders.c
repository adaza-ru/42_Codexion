...
static int	do_refactor(t_env *env, t_coder *coder)
{
...
static int	do_debug(t_env *env, t_coder *coder)
{
	if (check_simulation_end(env))
		return (0);
...
static int	do_compile(t_env *env, t_coder *coder)
{
	if (check_simulation_end(env))
		return (0);
...
static void	coder_loop(t_env *env, t_coder *coder)
{
	while (!check_simulation_end(env))
	{
		if (!take_dongles(coder))
			break ;
		if (!do_compile(env, coder))
			break ;
		release_dongles(coder);
		if (!do_debug(env, coder))
			break ;
		if (!do_refactor(env, coder))
			break ;
	}
}
